...
# 使用侧边栏实现多页面显示效果
with st.sidebar:
    st.image('images/rigth_logo.png', width=100)  # 修正原代码拼写错误rigth→right
    st.title('请选择页面')
    page = st.selectbox(
        "请选择页面", 
        ["简介页面", "预测分类页面"], 
...
# 预测分类页面逻辑
    ...
    with col_form:
        ...
        # 使用pickle加载预训练的随机森林模型
        with open('rfc_model.pkl', 'rb') as f:
            rfc_model = pickle.load(f)
        
        # 使用pickle加载物种编码与名称的映射对象
    ...
    with col_logo:
        if not submitted:
            # 未提交时显示logo
            st.image('images/rigth_logo.png', width=300)  # 修正原代码拼写错误rigth→right
        else:
            # 提交后显示对应企鹅物种的图片
